...
Antes de criar uma versão, atualize `version` em `package.json`, execute `npm run sync:version`, aumente `versionCode` em `android/app/build.gradle` e reconstrua o HTML offline. A CI verifica `npm run check:version` e `npm run check:offline`. Uma tag `v<versão>` dispara a compilação e publicação do APK assinado. A configuração inicial dos segredos e a guarda da chave estão em [docs/publicacao-android.md](docs/publicacao-android.md).
