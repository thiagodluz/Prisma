...
const html = read('index.html')
  ...
  .replace('<link rel="stylesheet" href="style.css?v=9">', `<style>${css}</style>`)
  .replace('<script type="module" src="app.js?v=9"></script>',
    `<script>${read('engine.js').replaceAll('export const ', 'const ').replace('export class ', 'class ')}\n${read('zen.js').replaceAll('export const ', 'const ').replaceAll('export function ', 'function ').replace('export class ', 'class ')}\n${read('sound.js').replace('export class ', 'class ').replace('export function ', 'function ')}\n${read('app.js').replace("import {Game, SIZE, levelGoal} from './engine.js?v=9';", '').replace("import {ZenAudio, normalizeZenSettings, breathTiming} from './zen.js?v=9';", '').replace("import {SoundDesign, cueForFrame} from './sound.js?v=9';", '').replace("if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js').catch(() => {});", '')}</script>`);
